...
class down_block_learned(tf.keras.layers.Layer):
     def __init__(self, nof_feature_maps = 16, kernel_size=3, strides=1, padding='same', pool_size=(2,2), dropout_rate = 0.0):
         super().__init__();
         self.c1 = Conv2D(nof_feature_maps, kernel_size, strides = strides, padding=padding);
         self.b1 = BatchNormalization();
         ...
         self.a2 = Activation('relu');
         self.dp2 = Dropout(0.0);
         self.d1  = Conv2D(nof_feature_maps, kernel_size, strides = strides*2, padding = padding);
...
class Channel_Mixer_3D(tf.keras.layers.Layer):
     def __init__(self, nof_feature_maps = 16, kernel_size=3):
         super().__init__();
         self.c1 = Conv3D(nof_feature_maps, kernel_size, strides = 1, padding='valid');
         self.b1 = BatchNormalization();
         self.a1 = Activation('relu');
         self.c2 = Conv3D(nof_feature_maps//2, kernel_size, strides = 1, padding='valid');
         self.b2 = BatchNormalization();
         self.a2 = Activation('relu');
         self.c3 = Conv3D(1, kernel_size, strides = 1, padding='valid');
...
class UNet(tf.keras.Model):
    def __init__(self):
        super().__init__();
        self.d0 = down_block(nof_feature_maps=16);
        self.d1 = down_block(nof_feature_maps=32);
        self.d2 = down_block(nof_feature_maps=64);
        self.d3 = down_block(nof_feature_maps=128);
        self.d4 = down_block(nof_feature_maps=256);
        self.d5 = down_block(nof_feature_maps=256, pool_size=(3,3));
        self.b1 = bottle_neck(nof_feature_maps=256);
        self.u0 = up_block(nof_feature_maps=256, upsample_size=(3,3) );
        self.u1 = up_block(nof_feature_maps=256);
        self.u2 = up_block(nof_feature_maps=128);
        self.u3 = up_block(nof_feature_maps=64);
        self.u4 = up_block(nof_feature_maps=32);
        self.u5 = up_block(nof_feature_maps=16);
        self.out_conv = Conv2D(1, (3,3), padding='same',strides=1);
...
####BIN
#Patchwise average over basis
"""
nof_patches_x, nof_patches_y = bh//patch_size[0], bw//patch_size[1];
basis         = tf.transpose(basis, perm = (0,3,4,1,2));
basis_patches = tf.reshape(basis, shape  = [ -1, bc,bf,nof_patches_x, patch_size[0], nof_patches_y, patch_size[1]]);
pet_features  = tf.expand_dims(pet_features,-1);
pet_features  = tf.transpose(pet_features, perm = (0,3,4,1,2));
pet_features  = tf.reshape(pet_features, shape  = [ -1, bc,1,nof_patches_x, patch_size[0], nof_patches_y, patch_size[1]]);
print('Basis patches: ', basis_patches.shape);
print('PET patches: ', pet_features.shape);
bPet           = basis_patches*pet_features; #[B,bc,1,Nx,x,Ny,y];
print('bPet: ', bPet.shape);
bPet_sum       = tf.math.reduce_sum(bPet, axis=(4,6), keepdims=True);
print('bPet sum: ', bPet_sum.shape);
basis_sum      = tf.math.reduce_sum(basis_patches,axis=(4,6), keepdims=True);
bPet           = tf.math.divide_no_nan(basis_patches*bPet_sum,basis_sum);
print('bPet: ', bPet.shape);
bPet           = tf.math.reduce_sum(bPet, axis=2, keepdims = True); #[B,Nx,x,Ny,y,1];
bPet           = tf.reshape(bPet, shape= [-1,bc,1,bh,bw]);
print('bPet out: ', bPet.shape);
bPet           = tf.transpose(bPet, perm = (0,3,4,1,2));
mr_features    = tf.expand_dims(mr_features,axis=-1);
stacked_feats  = tf.concat([mr_features,bPet],axis=-1);
#stacked_feats = tf.transpose(stacked_feats, perm = (0,1,2,4,3))
print('stacked feats: ', stacked_feats.shape);
mixed_feats    = self.conv_mixer3d(stacked_feats);
print('mixed feats: ', mixed_feats.shape);
mixed_feats    = tf.math.reduce_sum(mixed_feats,axis=3,keepdims=False);
print('summed mixed feats: ', mixed_feats.shape);
print('mixed feats: ', mixed_feats.shape);
for i in range(len(self.texture_blocks)):
    mixed_feats   = self.texture_blocks[i](mixed_feats);
"""
#Global average over MR basis
